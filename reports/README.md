...
|---|---|---|---|
| 2026-09-04 | [Image hardening: the graphical decode chain, and the libxml2 advisory](2026-09-04-image-hardening.md) | removing the graphical image-decoding stack a headless server never uses, and settling the three High libxml2 CVEs a scanner reports against the base | the decode chain (`libtiff`/`giflib`/`libjxl`/`librsvg`/`lcms2`/`gdk-pixbuf2`/`glycin` + `fontconfig`/`freetype2`/`libpng`) is provably removable with no loss of boot, snapshot or router acceptance; the three High libxml2 CVEs are already fixed in the installed `2.15.3` and the advisory listing them is stale |
| 2026-09-04 | [Upstream bump: omarchy v4.0.2](2026-09-04-upstream-4.0.2.md) | moving the pin from the default-branch tip to the release tag `v4.0.2`, packaging it and proving it on a booted machine | 19/19 patches apply, `make test` green, three packages built and signed, 85/85 packaging assertions, 16/17 router acceptance (the miss is the test machine's own leftover state) |
| 2026-08-31 | [Router profile: clean-machine acceptance attempt](2026-08-31-router-profile-clean-machine.md) | installing the `router` profile from its own ISO with `serverlab`, and the acceptance suite that measures it | ISO builds clean; a finding blocks the headless install — a no-roles single-NIC install locks itself out of SSH (firewall opens 22 on `wan0` only, and no NIC is `wan0` without roles). Posture decision pending |
| 2026-08-30 | [The router profile](2026-08-30-router-profile.md) | a `router` edition beside `server`: two NIC roles (`wan0`/`lan0`), IP forwarding, and an nftables `inet tui` firewall exposing only SSH and WireGuard, no web port | static only — 13/13 patches apply, orchestrator compiles, every leaf parses; the router-1.0 acceptance on real NICs (items 1-7) is owed to the lab |
| 2026-08-30 | [The demo image: tui-tools from their own signed repository](2026-08-30-oci-demo-tui-tools.md) | `fakeroot` in the base, the `tui-tools` addon installing fourteen terminal UIs from their own signed repository, and the cloud image built for a reviewer to log into | cloud 22/22, cloud SELinux 42/42, packaging 79/79; one build-driver bug found and fixed — an image could be built out of an earlier run's disk |
| 2026-08-29 | [Transactional updates under SELinux enforcing](2026-08-29-transactional-selinux.md) | a fresh `--mac selinux` install taken to enforcing by the guarded command, then the transactional list and the SELinux list against the same machine | SELinux 45/45 permissive; transactional 28/6 (four of the six inapplicable without Secure Boot); enforcing pass 55/4. Three defects found, **two fixed and re-measured** — one transaction used to unmount the live `/sys/fs/selinux` |
