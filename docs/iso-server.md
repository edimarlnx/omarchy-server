...
| `0013-cidata-router-nics.patch` | `usr/local/bin/omarchy-cidata-load` | Accepts a `router-nics` file on the `cidata` drive (`WAN_MAC=`/`LAN_MAC=` lines), so an autoinstalled router can be told which physical NIC is the WAN and which is the LAN. `install/router/network-router.sh` copies it to `/etc/omarchy/router/roles.conf` and `omarchy-router-nics` turns it into the systemd `.link` files that rename them to `wan0`/`lan0`. Applied last so it extends the fully built `optional_inputs` list. |
